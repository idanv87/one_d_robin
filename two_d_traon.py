...
# lsof -i:6006 
# kill -9 68614
# tensorboard --logdir=/Users/idanversano/Documents/project_geo_deeponet/one_d_robin/runs/
...
# best_model=torch.load(Constants.path+'runs/'+'2024.04.16.11.07.11best_model.pth')
...
def predict(model, dataloader, dataset, criterion):
    ...
    with torch.no_grad():
        ...
        if epoch % 20 ==0:
            ...
            plt.title(np.linalg.norm(torch.cat(prediction, axis=0)-torch.cat(y_test, axis=0))/
                      np.linalg.norm(torch.cat(y_test, axis=0)))
            plt.scatter(torch.cat(coords, axis=0), torch.cat(prediction, axis=0))
            plt.scatter(torch.cat(coords, axis=0), torch.cat(y_test, axis=0),color='r')
